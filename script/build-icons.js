...
import mapicons from "../lib/build/data.json" assert { type: "json" };
...
const srcDir = resolve("components/__generated__");
...
const icons = Object.entries(mapicons)
  .map(([key, octicon]) => {
          ...
          t.addComment(
            t.callExpression(t.identifier("createIconComponent"), [
              ...
              t.arrowFunctionExpression(
                [],
                t.blockStatement([t.returnStatement(svgData)])
              ),
            ]),
            "leading",
            "#__PURE__"
...
function svgToJSX(node) {
  ...
  if (node.type === "text") return t.jsxText(node.value);
